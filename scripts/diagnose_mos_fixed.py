...
def run_episode(
    *,
    ...
    layout = layout or MOSLayout(map_seed=map_seed)
    if layout.map_seed != map_seed:
        raise ValueError("layout.map_seed must match map_seed")
        ...
        decision_rows.append(
            {
                ...
                "start_x": layout.start[0],
                "start_y": layout.start[1],
                "sensor_range": layout.sensor_range,
...
def summarize(episodes: list[dict], decisions: list[dict]) -> dict:
    ...
    def instance_key(row):
        if row.get("instance_seed") is not None:
            return ("instance", row["instance_seed"])
        return (row["map_seed"], row["target_seed"], row["observation_seed"])
    ...
    for configuration in sorted(grouped_episodes):
        ...
        successes = sum(row["success"] for row in episode_values)
        decision_times = [row["total_ms"] for row in decision_values]
        configurations.append(
            {
                ...
                "success_rate": successes / len(episode_values),
                "success_rate_ci95_wilson": wilson_interval(successes, len(episode_values)),
    ...
    for row in decisions:
        instance = (*instance_key(row), row["resolution"])
    ...
    for resolution in sorted({row["resolution"] for row in episodes}):
        matched = [
            ...
            if instance[-1] == resolution and all(depth in sequences for depth in (1, 2, 3))
        ]
